...
class AFileManager(AObject):
        ...
        if path:
            if os.path.isfile(path):
                ...
                if os.path.isfile(self.getJSONPath()):
                    ...
                    # no json file found, so we create one
            else:
                assert (
                    False
    ...
    @staticmethod
    def copyRandomFractionOfFilesInSourceDir(
        source_dir, dest_dir, fraction=1.0, ext=None
    ):
        """
        ...
        for d in range(len(directories)):
            ...
            AFileManager.copyRandomFractionOfFilesInSourceDir(
                source_dir=directories[d], dest_dir=subdest, fraction=fraction, ext=ext
            )
