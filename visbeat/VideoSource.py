...
class VideoSource(AFileManager):
    ...
    def __init__(
        self, path, name=None, source_location=None, VideoClass=None, **kwargs
    ):
        """
    ...
    def getVersionPath(self, version_label=None, version_group=None):
        ...
        return self.getVersionInfo(
            version_label=version_label, version_group=version_group, info_label="path"
        )
    ...
    def setVersionInfo(
        self, version_label=None, version_group=None, video_path=None, **kwargs
    ):
        version_dict = self.getVersionDictionary(
            version_label=version_label, version_group=version_group
        )
        if version_dict is None:
            self.setVersionDictionary(
                version_label=version_label, version_group=version_group, d={}
            )
            version_dict = self.getVersionDictionary(
                version_label=version_label, version_group=version_group
            )
    ...
    def getVersion(
        self, max_height=None, get_if_missing=True, load_features=True, **kwargs
    ):
        """
        ...
        if vpath and os.path.isfile(vpath):
            num_frames_total = self.getVersionInfo(
                version_label=max_height, info_label="num_frames_total"
            )
            if num_frames_total:
                v = self.RegisteredVideo(
                    path=vpath,
                    version_label=max_height,
                    num_frames_total=num_frames_total,
                    load_features=True,
                )
            else:
                v = self.RegisteredVideo(
                    path=vpath, version_label=max_height, load_features=True
                )
                self.setVersionInfo(
                    version_label=max_height, num_frames_total=v.num_frames_total
                )
                self.save()
            return v
        else:
            ...
            if get_if_missing:
                ...
                if vpath:
                    ...
                    if num_frames_total:
                        ...
                        self.setVersionInfo(
                            version_label=max_height,
                            num_frames_total=v.num_frames_total,
                        )
                        self.save()
                else:
    ...
    def saveFeaturesForVideo(
        self, video, features_to_save=None, output_dir=None, overwrite=True
    ):
        ...
        for f in features_to_save:
            self.saveFeatureForVideo(
                video=video, feature_name=f, output_dir=output_dir, overwrite=overwrite
            )

    def saveFeatureForVideo(self, video, feature_name, output_dir=None, overwrite=True):
        if feature_name == "each":
            return self.saveFeaturesForVideo(
                video=video,
                features_to_save=video.getFeaturesList(),
    ...
    def loadFeatureForVideo(self, video, feature_name):
        if feature_name == "each":
            return self.loadFeaturesForVideo(
                video=video, features_to_load=video.getFeatureFunctionsList()
            )
        version_label = video.getInfo(label="version_label")
        source_type = video.getFeatureSourceType(feature_name)
        ipath = self.getFeaturePath(
            feature_name=feature_name,
            version_label=version_label,
            source_type=source_type,
        )
    ...
    def getFeaturePath(
        self, feature_name=None, source_type=None, version_label=None, output_dir=None
    ):
        assert (
            feature_name is not None
        ), "must provide name of feature VideoSource.getFeaturePath"
    ...
    def RegisteredVideo(
        ...
        v = self.VideoClass(
            ...
            name=self.getName()
            + "_"
            + self._versionLabelString(version_label=version_label),
            num_frames_total=num_frames_total,
        )
        ...
        self.RegisterVideo(
            video=v, version_label=version_label, load_features=load_features
        )
        return v

    def RegisterVideo(
        self, video, version_label=None, version_group=None, load_features=True
    ):
        def videosave(vidob, features_to_save="all", output_dir=None, overwrite=True):
            self.saveFeaturesForVideo(
                video=vidob,
                features_to_save=features_to_save,
                output_dir=output_dir,
                overwrite=overwrite,
        ...
        def videoclear(vidob, features_to_clear="all"):
            self.removeFeatureFilesForVideo(
                video=vidob, features_to_remove=features_to_clear
            )
        ...
        elif load_features is not None and (
            isinstance(load_features, list) or isinstance(load_features, str)
        ):
    ...
    def addVersionToVideo(
        self, video, new_video, version_label=None, version_group=None
    ):
        version_label = video.getInfo(label="version_label")
        ...
        if (version_group is not None) or (video_dict.get("version_group") is None):
            video_dict.update(dict(version_group=version_group))
        self.setVersionInfo(
            video_path=new_video.getPath(), version_label=version_label, **video_dict
        )
        self.save()
    ...
    def pullFileVideo(self, max_height=None, force_recompute=None):
        ...
        assert os.path.isfile(original_path), "SOURCE FILE {} IS MISSING".format(
    ...
    def pullYoutubeVideo(
        self,
        max_height=None,
        write_subtitles=False,
        save_youtube_info=False,
        ...
        :return:
        """
        assert not self.getInfo(
            "source_is_file"
        ), "tried to call pullYoutubeVideo on file source {}.".format(self.getName())
        ...
        if (not force_redownload) and old_vid_path and os.path.isfile(old_vid_path):
            AWARN(
                "Old video version exists with path {}\nSkipping download...\n".format(
                    old_vid_path
                )
            )
            return old_vid_path
        ...
        if max_height:
            ydl_opts[
                "format"
            ] = "bestvideo[height<={}][ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best".format(
                max_height
            )
        ...
        if os.path.isfile(filepath):
            ...
            jpath = glob.glob(
                output_dir + os.sep + "*" + info_dict["id"] + ".info.json"
            )
