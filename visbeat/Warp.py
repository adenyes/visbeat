...
class Warp(AObject):
    ...
    def setWarpFunc(self, warp_type, **kwargs):
        if warp_type == "square":
            ...
            self.warp_func = [
                Warp.GetEventWarpFunc(
                    self.source_events,
                    self.target_events,
                    Warp.WFunc_Quadratic(),
                    **kwargs
                ),
                Warp.GetEventWarpFunc(
                    self.target_events,
            ...
            self.warp_func = [
                Warp.GetEventWarpFunc(
                    self.source_events,
                    self.target_events,
                    Warp.WFunc_Weight(use_to_weights=None, **kwargs),
                    **kwargs
                ),
                Warp.GetEventWarpFunc(
                    self.target_events,
                    self.source_events,
                    Warp.WFunc_Weight(use_to_weights=True, **kwargs),
                    **kwargs
                ),
            ]
            ...
            self.warp_func = [
                Warp.GetEventWarpFunc(
                    self.source_events, self.target_events, Warp.WFunc_P(p=p), **kwargs
                ),
                Warp.GetEventWarpFunc(
                    self.target_events, self.source_events, Warp.WFunc_P(p=p), **kwargs
                ),
            ]
    ...
    def warpSourceTime(self, t):
        return self.warp_func[0](
            t, a_events=self.source_events, b_events=self.target_events
        )
    ...
    def warpTargetTime(self, t):
        return self.warp_func[1](
            t, a_events=self.target_events, b_events=self.source_events
        )
    ...
    def plotImage(self, xlim=None, sampling_rate=None):
        ...
        new_n_samples = target_duration * sampling_rate
        target_start_times = np.linspace(
            target_start, target_end, num=new_n_samples, endpoint=True
        )
        ...
        unwarped_target_times = np.array(unwarped_target_times)
        pim = Image.PlotImage(
            signal=unwarped_target_times,
            show_axis=True,
            xvals=target_start_times,
    ...
    def repeatShorterEvents(self, endpoints=False):
        ...
        self.source_events = Event.RepeatToLength(
            self.source_events, n=n_events, endpoints=endpoints
        )
        self.target_events = Event.RepeatToLength(
            self.target_events, n=n_events, endpoints=endpoints
        )
    ...
    def LinearInterp(t, a_events, b_events):
        ...
        return (next_weight * next_b_event_time) + (
            (1.0 - next_weight) * prev_b_event_time
        )
    ...
    def GetEventWarpFunc(from_events, to_events, f, lead_time=None, **kwargs):
        ...
        def rfunc(t, **kwargs):
            ...
            if next_f_event_index == 0:
                from_cap_event = Event(
                    start=f_events[0].start - start_cap_time, weight=0, type="startcap"
                )
                to_cap_event = Event(
                    start=t_events[0].start - start_cap_time, weight=0, type="startcap"
                )
                return f(
                    t,
                    f_neighbors=[from_cap_event, f_events[0]],
                    t_neighbors=[to_cap_event, t_events[0]],
                )
                ...
                return f(
                    t,
                    f_neighbors=[
                        f_events[next_f_event_index - 1],
                        f_events[next_f_event_index],
                    ],
                    t_neighbors=[
                        t_events[next_f_event_index - 1],
                        t_events[next_f_event_index],
                    ],
                )
    ...
    def WFunc_Mouth(p_acceleration_time=0.1, **kwargs):
        def rfunc(t, f_neighbors, t_neighbors):
            ...
            if (
                f_neighbors[1].type == "mouth_open"
                or t_neighbors[1].type == "mouth_open"
            ):
                ...
                if p < 0:
                    ...
                    if progress_frac < p:
                        ...
                        next_weight = (a * progress_frac) + np.power(
                            (progress_frac - p), 2.0
                        )
            else:
    ...
    def WFunc_targettime_sourcefraction(
        acceleration_target_time=0.1, acceleration_source_fraction=0.8, **kwargs
    ):
        """
        ...
        def rfunc(t, f_neighbors, t_neighbors):
            ...
            if from_event_gap == 0:
                AWARN("Event Gap was 0! Check Warp.py")
                return to_times[0] + t_progress
                # is this right? doesnt seem to come up, not sure what behavior should be looking back on it...
            ...
            if acceleration_target_time >= from_event_gap or q > (p2):
                ...
                lin_t_progress_frac = truediv(
                    t_progress, from_event_gap - acceleration_target_time
                )
    ...
    @staticmethod
    def WFunc_target_source_fractions(
        acceleration_target_fraction=0.8, acceleration_source_fraction=0.9, **kwargs
    ):
        """
        This assumes that you are mapping from the target to the source, as is the most common use case.
    ...
    def ABWarp():
        ...
        next_weight = math.pow(progress_frac, 2)
        return (next_weight * next_to_event_time) + (
            (1.0 - next_weight) * prev_to_event_time
        )
    ...
    def LinearInterpFunc(a_events, b_events):
        ...
        return sp.interpolate.interp1d(
            ae, be, "linear", bounds_error=False, fill_value="extrapolate"
        )
    ...
    def CubicInterpFunc(a_events, b_events):
        ...
        splineX = sp.interpolate.interp1d(
            spline_times,
            wspline_times,
            kind="cubic",
            bounds_error=False,
