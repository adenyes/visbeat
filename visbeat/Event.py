...
class Event(AObject):
    ...
    def _getPhase(self, phase_resolution=None):
        ...
        if phase_resolution is None:
            ...
            return self.getInfo("phase") * (
                phase_resolution / Event._EVENT_PHASE_BASE_RES
            )
    ...
    def _setPhase(self, phase, phase_resolution):
        if phase_resolution is None:
            ...
            self.setInfo(
                "phase", phase * (Event._EVENT_PHASE_BASE_RES / phase_resolution)
            )
    ...
    def GetUnrolledList(event_list, assert_on_folds=None):
        ...
        for e in range(1, len(event_list)):
            ...
            if assert_on_folds:
                assert (
                    newe.start >= out_list[-1].start
                ), "FOLD (non-monotonic event list) DETECTED WHEN NOT ALLOWED!!!\n see Event.GetUnrolledList"
            newe.unrolled_start = out_list[-1].unrolled_start + np.fabs(
                newe.start - out_list[-1].start
            )
            newe.index = e
    ...
    def FromStartsAndWeights(cls, starts, weights, type=None):
        ...
        assert len(starts) == len(
            weights
        ), "Event.FromStartsAndWeights got {} starts and {} weights".format(
            len(starts), len(weights)
        )
    ...
    @staticmethod
    def FromSignalPeaks(
        signal, sampling_rate, event_type=None, index_offset=None, **kwargs
    ):
        xvals = np.arange(len(signal))
        xvals = xvals * truediv(1.0, sampling_rate)
        ...
        return Event.FromStartsAndWeights(
            starts=peaktimes, weights=peakvals, type=event_type
        )
    ...
    def GetScaled(events, scale):
        ...
        for e in events:
            oute.append(
                Event(e.start * scale, weight=e.weight, type=e.type, index=e.index)
            )
        return oute
    ...
    def ClosestToTargetMatch(source_events, target_events):
        ...
        for ti in range(len(S_sorted)):
            ...
            matched_source_event_i = S_sorted[ti][0][0]
            # sorted lists, first element, index value for index
            existing_match = match_map.get(matched_source_event_i)
            matchdist = math.fabs(
                source_events[matched_source_event_i].start - target_events[ti].start
            )
    ...
    def Sort(event_list, func=None):
        assert (
            func is None or func == "start" or func == "time"
        ), "have not implemented sort by {}".format(func)
    ...
    @staticmethod
    def PlotSignalAndEvents(
        signal, sampling_rate, events, time_range=None, ylim=None, **kwargs
    ):
