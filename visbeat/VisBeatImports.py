...
try:
    ...
    AWARN(
        "matplotlib problem... if you are using conda try installing with 'conda install matplotlib'"
    )
    matplotlib.use("agg")
    import matplotlib.pyplot as plt
...
def get_hist_clipped(signal, clip_bins=30, clip_fraction=0.95):
    ...
    while total_included < clip_fraction and bins_included < clip_bins:
        if (prevbini >= 0 and sigh[prevbini] == 0) and (
            nextbini < (clip_bins) and sigh[nextbini] == 0
        ):
            prevbini = prevbini - 1
            nextbini = nextbini + 1
        else:
        ...
        icounter += 1
    clipsig = np.clip(
        a=sigrav, a_min=sigb[max(prevbini, 0)], a_max=sigb[min(nextbini, clip_bins)]
    )
