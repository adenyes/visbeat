...
class VisualBeat(Event):
    ...
    def clone(self, start=None):
        if start:
            newv = VisualBeat(
                start=start, type=self.type, weight=self.weight, index=self.index
            )
        else:
            newv = VisualBeat(
                start=self.start, type=self.type, weight=self.weight, index=self.index
            )
    ...
    def FromEvent(e):
        if isinstance(e, VisualBeat):
            ...
            return VisualBeat(
                start=e.start, type=e.type, weight=e.weight, index=e.index
            )
        ...
        def objective_func(a, b):
            ...
            if absolute:
                return binary_weight * (
                    np.fabs(np.dot(a.flow_histogram, b.flow_histogram)) - 0.70710678118
                )
                # cos 45 degrees
            else:
                return binary_weight * (np.dot(a.flow_histogram, b.flow_histogram))
                # cos 45 degrees
    ...
    def PullOptimalPaths_Basic(
        ...
        binary_objective = VisualBeat.tempo_binary_objective(
            target_period=target_period, binary_weight=binary_weight
        )
        unary_objective = VisualBeat.weight_unary_objective(unary_weight=unary_weight)
        window_function = VisualBeat.time_window_func(
            max_separation=window_size, break_on_cuts=break_on_cuts
        )
        return VisualBeat.DynamicProgramOptimalPaths(
            vis_beats=vis_beats,
    ...
    def PullOptimalPaths(
        ...
        if binary_fn == "autocor":
            binary_objective = VisualBeat.autocor_binary_objective(
                binary_weight=binary_weight
            )
        elif binary_fn == "angle":
            binary_objective = VisualBeat.angle_binary_objective(
                binary_weight=binary_weight
            )
        else:
            binary_objective = VisualBeat.tempo_binary_objective(
    ...
    @staticmethod
    def DynamicProgramOptimalPaths(
        vis_beats, unary_objective_func, binary_objective_func, window_func
    ):
        class Node(object):
        ...
        for n in range(1, len(nodes)):
            ...
            if len(options) == 0:
                ...
                best_score = options[0].cum_score + binary_objective_func(
                    current_node.object, best_choice.object
                )
                for o in range(1, len(options)):
                    score = options[o].cum_score + binary_objective_func(
                        current_node.object, options[o].object
                    )
                    if score > best_score:
                        best_choice = options[o]
                        best_score = score
                current_node.prev_node = best_choice
                current_node.cum_score = best_score + unary_objective_func(
                    current_node.object
                )
