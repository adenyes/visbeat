...
class Audio(TimeSignal1D):
    ...
    def loadFile(self, file_path=None, sampling_rate=None, convert_to_mono=True):
        ...
        if "file_path" in self.a_info:
            self.x, self.sampling_rate = librosa.load(
                self.a_info["file_path"], sr=sampling_rate, mono=convert_to_mono
            )
    ...
    def play(self, autoplay=None):
        ...
        if ISNOTEBOOK:
            # if(normalize):
            audiodisp = vb_get_ipython().display.Audio(
                data=self.getSignal(), rate=self.sampling_rate, autoplay=autoplay
            )
    ...
    def AudioClipFromBeatRange(self, beat_range, beats=None):
        ...
        return self.AudioClip(
            start=beats[beat_range[0]].start, end=beats[beat_range[1]].start
        )
    ...
    def playSegment(self, time_range, autoplay=None):
        ...
        if ISNOTEBOOK:
            audiodisp = vb_get_ipython().display.Audio(
                data=self.getSignalSegment(time_range=[start_time, end_time]),
                rate=self.sampling_rate,
                autoplay=autoplay,
            )
            vb_get_ipython().display.display(audiodisp)
    ...
    def AlignedTo(self, B):
        assert (
            False
        ), "Abe needs to fix -- broke when messing with alignment code for video"
    ...
    def getWithSoundAdded(
        self, add_times, sound=None, mute_original=None, gain_original=None
    ):
        if sound is None:
    ...
    def showMelSpectrogram(self, force_recompute=False, **kwargs):
        ...
        librosa.display.specshow(
            mel_spec, sr=self.sampling_rate, x_axis="time", y_axis="mel"
        )
    ...
    @staticmethod
    def _getDampedSin(
        freq, n_seconds=None, sampling_rate=16000, damping=0.1, noise_floor=0.001
    ):
        """
        ...
        x = np.sin(
            np.linspace(
                0, n_seconds * freq * 2 * np.pi, np.round(n_seconds * sampling_rate)
            )
        )
    ...
    def PingSound(n_seconds=None, freqs=None, damping=None, sampling_rate=16000):
        if freqs is None:
            ...
            freqs = np.arange(5, 25) * 75
            # just kind of thought this sounded fine...
        if damping is None:
            damping = [0.05] * len(freqs)
        if not isinstance(damping, list):
            damping = [damping] * len(freqs)
        s = Audio._getDampedSin(
            freq=freqs[0],
            n_seconds=n_seconds,
    ...
    def getOnsets(self, use_full_signal=True, force_recompute=False, **kwargs):
        ...
        if (not self.hasFeature(name="onsets")) or force_recompute:
            if use_full_signal:
                ...
                onsets = librosa.onset.onset_detect(
                    y=self.getSignal(), sr=self.sampling_rate, units="time", **kwargs
                )
                self.setFeature(name="onsets", value=onsets)
    ...
    def getOnsetEnvelope(
        self, use_full_signal=True, force_recompute=False, centering=True, **kwargs
    ):
        """use_full_signal only makes a difference in AudioClip subclass."""
    ...
    def getMelSpectrogram(self, n_mels=128, force_recompute=False):
        ...
        if (not self.hasFeature(name=feature_name)) or force_recompute:
            params = dict(sr=self.sampling_rate, n_mels=n_mels)
            Spec = librosa.feature.melspectrogram(self.getSignal(), **params)
            self.setFeature(
                name=feature_name,
                value=librosa.power_to_db(Spec, ref=np.max),
                params=params,
            )
    ...
    def getRMSE(self, force_recompute=False, hop_length=None, frame_length=None):
        ...
        if (not self.hasFeature(name=feature_name)) or force_recompute:
            ...
            params = dict(hop_length=hop_length, center=True, frame_length=frame_length)
            rmse = librosa.feature.rmse(y=self.getSignal(), **params)
            self.setFeature(
                name=feature_name, value=np.ndarray.flatten(rmse), params=params
            )
        return self.getFeature(feature_name)
    ...
    def getTempogram(
        ...
        **kwargs
        ...
        if (not self.hasFeature(feature_name)) or force_recompute:
            ...
            tparams = dict(
                y=y, sr=sr, hop_length=hop_length, win_length=win_length, **kwargs
            )
    ...
    def plotTempogram(self, window=None, time_range=None, **kwargs):
        ...
        mplt = librosa.display.specshow(
            toshow,
            sr=tparams["sr"],
            hop_length=tparams["hop_length"],
            x_axis="time",
            y_axis="tempo",
        )
    ...
    def plotOnsets(self, **kwargs):
        ...
        mplt = Event.PlotSignalAndEvents(
            signal, sampling_rate=self.getOnsetSamplingRate(), events=events, **kwargs
        )
        plt.xlabel("Time (s)")
