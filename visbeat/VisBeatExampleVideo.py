...
class VisBeatExampleVideo(object):
    ...
    def _fancyBoxCode(self, with_label=None):
        ...
        if with_label:
            s.addLine("<figure>")
        s.addLine(
            '<a class="vb_youtube_fancybox" data-fancybox href="{watchurl}"><img alt="{alt_name}" src="{thumburl}" /></a>'.format(
                watchurl=self._ytWatchURL(),
...
class HTMLCode(object):
    ...
    def startTable(self, id=None, class_=None, **kwargs):
        ...
        s = (
            s
            + 'width="{table_width}" border="{border}" cellspacing="{cellspacing}" cellpadding="{cellpadding}">'.format(
                **targs
            )
        )
        self.addLine(s)
