...
def PullVideo(name=None, source_location=None, max_height=240, **kwargs):
    if isinstance(name, VisBeatExampleVideo):
        ...
        ), "Provided VisBeatExampleVideo and source location? What are you trying to do?"
...
def ClipVideo(video, time_range, max_height=240):
    ...
    vcdir = video.source.getDirForVersion(
    ...
    **kwargs
    ...
    if nbeats:
        ...
        if vbeats[-1].start < source_video.getDuration():
            ...
            print(
                "start_video_beat: {}, start_audio_beat: {}".format(
            ...
            target_newbeat.start = min(
                target_newbeat.start + deltatime, target_audio.getDuration()
            )
    ...
    if output_path:
        ...
        if os.path.isfile(final_output_path):
            ...
            final_output_path = os.path.join(
                output_directory_path, tryname + output_ext
            )
        ...
        warped_result_final = Video(
            path=final_output_path, num_frames_total=n_frames_total
        )
