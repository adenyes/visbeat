...
if USING_OPENCV:
            ...
            for dx in range(n_shifts[1]):
                ...
                for x in range(nperd):
                    for y in range(nperd):
                        ...
                        cahis, cbinbounds = np.histogram(
                            angcell.ravel(),
                            bins=bins,
                            range=(0, 2 * np.pi),
                            weights=ampcell.ravel(),
                            density=density,
                        )
                        # print("ahis shape: {}\ncahis shape: {}\ndx, dy: {}, {}\ncell_counter: {}\nbins: {}\n".format(ahis.shape, cahis.shape, dx, dy, cell_counter, bins))
                        ahis[
                            cell_counter * bins : (cell_counter + 1) * bins,
                            dx + dy * n_shifts[0],
                        ] = cahis
    ...
    def computeDirectogramPowers(
        self,
        bins=None,
        dead_zone=0.05,
        density=None,
        save_if_computed=True,
        noise_floor_percentile=None,
        **kwargs
    ):
        if bins is None:
            bins = 128
        ...
        for nf in range(len(frame_index_floats)):
            ...
            ahis1, cbinbounds1 = np.histogram(
                angw.ravel(),
                bins=bins,
                range=(0, 2 * np.pi),
                weights=ampw.ravel(),
            ...
            ahis2, cbinbounds2 = np.histogram(
                angw.ravel(),
                bins=bins,
                range=(0, 2 * np.pi),
                weights=np.power(ampw, 2).ravel(),
                density=density,
            )
    ...
    def getVisualTempo(self, force_recompute=None, **kwargs):
        ...
        if (not self.hasFeature(feature_name)) or force_recompute:
            ...
            result = librosa.beat.estimate_tempo(
                onset_envelope=vbe, sr=self.sampling_rate, hop_length=1, **kwargs
            )
            self.setFeature(name=feature_name, value=result, params=kwargs)
        return self.getFeature(feature_name)

    def getVisualTempogram(
        self, window_length=None, force_recompute=None, norm_columns=None, **kwargs
    ):
        """
        ...
        if (not self.hasFeature(feature_name)) or force_recompute:
            ...
            odf_frame = librosa.util.frame(
                onset_envelope, frame_length=win_length, hop_length=hop_length
            )
            ...
            if norm_columns:
                ...
                result = librosa.util.normalize(
                    librosa.core.audio.autocorrelate(
                        odf_frame * ac_window[:, np.newaxis], axis=0
                    ),
                    norm=norm,
                    axis=0,
                )
            else:
                result = librosa.core.audio.autocorrelate(
                    odf_frame * ac_window[:, np.newaxis], axis=0
                )
                result = np.true_divide(result, np.max(result.ravel()))

            tempo_bpms = librosa.tempo_frequencies(
                result.shape[0], hop_length=hop_length, sr=sr
            )
    ...
    def visualBeatsFromEvents(self, events):
        ...
        for e in events:
            ...
            b.flow_histogram = downsample_hist(
                sig=histslice, levels=HISTOGRAM_DOWNSAMPLE_LEVELS
            )
    ...
    def getDirectionalFlux(
        self, f_sigma=None, median_kernel=None, power=None, **kwargs
    ):
        """
    ...
    def computeImpactEnvelope(
        ...
        **kwargs
        ...
        inputargs = dict(
            f_sigma=f_sigma, median_kernel=median_kernel, power=power, crop=crop
        )
    ...
    def getVisibleImpacts(
        self, force_recompute=False, include_cut_events=None, **kwargs
    ):
        ...
        if (not self.hasFeature(feature_name)) or force_recompute:
            ...
            peak_params.update(kwargs)
            # if params given in arguments, those will override the defaults here.

            v_events = Event.FromSignalPeaks(
                signal=svbe, sampling_rate=u_sampling_rate, **peak_params
            )
    ...
    def getForwardVisibleImpacts(
        ...
        if (not self.hasFeature(feature_name)) or force_recompute:
            ...
            u_sampling_rate = self.sampling_rate * upsample_factor
            v_events = Event.FromSignalPeaks(
                signal=local_saliency,
                sampling_rate=u_sampling_rate,
                event_type="forward",
                **kwargs
            )
            if include_cut_events:
    ...
    def getBackwardVisibleImpacts(
        self, force_recompute=False, include_cut_events=None, **kwargs
    ):
        feature_name = "backward_visual_beats"
        if (not self.hasFeature(feature_name)) or force_recompute:
            params = kwargs
            local_saliency = self.getFeature(
                "backward_visual_impact_envelope", **kwargs
            )
            upsample_factor = kwargs.get("upsample_factor")
            if upsample_factor is None:
                upsample_factor = 1
            u_sampling_rate = self.sampling_rate * upsample_factor
            v_events = Event.FromSignalPeaks(
                signal=local_saliency, sampling_rate=u_sampling_rate, **kwargs
            )
            if include_cut_events:
                cut_events = self.getFeature("cut_events")
                v_events = v_events + cut_events
    ...
    def findAccidentalDanceSequences(
        self, target_n_beats=7, n_samples=25, delta_range=None
    ):
        if delta_range is None:
            delta_range = [0.02, 0.5]

        deltas = np.linspace(
            delta_range[0], delta_range[1], num=n_samples, endpoint=True
        )
        deltapick = delta_range[0]
        sequences = []
        for i, delta in enumerate(deltas):
            peak_vars = self._getDefaultPeakPickingTimeParams(delta=delta)
            sequences = self.getVisualBeatSequences(
                peak_vars=peak_vars, print_summary=False
            )
    ...
    def getVisualBeatSequences(
        ...
        **kwargs
        ...
        if search_tempo is not None:
            ...
            sequences = VisualBeat.PullOptimalPaths_Basic(
                impacts,
            ...
            sequences = VisualBeat.PullOptimalPaths_Autocor(
                impacts,
                unary_weight=unary_weight,
                binary_weight=binary_weight,
                break_on_cuts=break_on_cuts,
    ...
    def printVisualBeatSequences(
        ...
        **kwargs
        ...
        Event.PlotSignalAndEvents(
            self.getFeature("impact_envelope"),
            sampling_rate=self.sampling_rate * VB_UPSAMPLE_FACTOR,
            events=rsegments[0],
            time_range=time_range,
        )
        return rsegments
    ...
    def plotEvents(self, events, time_range="default", **kwargs):
        ...
        mplt = Event.PlotSignalAndEvents(
            signal,
            sampling_rate=self.sampling_rate * VB_UPSAMPLE_FACTOR,
            events=events,
            time_range=time_range_use,
            **kwargs
        )
        plt.xlabel("Time (s)")
        return mplt
