...
class VideoClip(Video):
    ...
    def __init__(self, video=None, start=None, end=None, clip_to_frame=True, path=None):
        ...
        assert not (
            video and path
        ), "provided both video object and path to VideoClip constructor."
        assert (start is not None) and (
            end is not None
        ), "must provide start time and end time to AudioClip constructor"
        ...
        if video:
            ...
            if clip_to_frame:
                self.start = time_per_frame * math.floor(
                    truediv(self.start, time_per_frame)
                )
                self.end = time_per_frame * math.floor(
                    truediv(self.end, time_per_frame)
                )
            if video.name:
    ...
    def getFrameLinearInterp(self, f):
        return Video.getFrameLinearInterp(
            self, float(f) + self.start * self.sampling_rate
        )
    ...
    def getMPYClip(self, get_audio=True):
        return mpy.VideoFileClip(self.getPath(), audio=get_audio).subclip(
    ...
    def play(self):
        if ISNOTEBOOK:
            ...
            IPython.display.display(
                self.getMPYClip().ipython_display(
                    fps=self.sampling_rate, maxduration=self.getDuration() + 1
                )
            )
        else:
