...
class VBObject(AObject):
    ...
    def getFeature(self, name, force_recompute=False, **kwargs):
        ...
        return self.features.getValue(
            name=name, params=kwargs, force_recompute=force_recompute
        )

    def getFeatureEntry(self, name, params=None, force_recompute=False):
        return self.features.getEntry(
            name=name, params=params, force_recompute=force_recompute
        )
    ...
    def removeFeature(self, name, assert_if_absent=True, set_modified=True):
        self.features.removeEntry(
            name=name, assert_if_absent=assert_if_absent, set_modified=set_modified
        )
    ...
    def clearFeatureFiles(self, features_to_clear=None, **kwargs):
        if self.clear_feature_files_func:
            self.clear_feature_files_func(
                self, features_to_clear=features_to_clear, **kwargs
            )
        else:
            VBWARN(
                "CLEAR FEATURE FILES FUNCTION HAS NOT BEEN PROVIDED FOR {} INSTANCE".format(
                    self.VBOBJECT_TYPE()
                )
            )
