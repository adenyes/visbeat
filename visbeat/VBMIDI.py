...
class VBMIDITrack(object):
    ...
    def get_note_durations(self, track_num):
        ...
        for msg in self.mido_track:
            if not msg.is_meta:
                ...
                if (
                    msg.type == "note_on"
                    and msg.velocity > 0
                    and msg.note not in currently_played_notes
                ):
        ...
        assert not bool(
            currently_played_notes
        ), "Finished looping through all messages. There should not be any notes playing at this point."
    ...
    def getNoteOnTimesAsAudio(
        self, sampling_rate=None, note_sound=None, n_seconds=None
    ):
        ...
        s = Audio.Silence(
            n_seconds=n_seconds, sampling_rate=sampling_rate, name=self.name
        )
...
class VBMIDI(TimeSignal1D):
    ...
    def getNoteOnTimesAsAudio(self, sampling_rate=None, note_sound=None):
        s = self.tracks[-1].getNoteOnTimesAsAudio(
            sampling_rate=sampling_rate,
            note_sound=note_sound,
            n_seconds=self.midi_file.length,
        )
        if s.name is None:
