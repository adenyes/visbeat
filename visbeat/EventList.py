...
class EventList(AObject):
    ...
    def unroll(self, assert_on_folds=None):
        newlist = Event.GetUnrolledList(
            event_list=self.events, assert_on_folds=assert_on_folds
        )
        self.events = newlist

    def getUnrolled(self, assert_on_folds=None):
        return EventList(
            Event.GetUnrolledList(
                event_list=self.events, assert_on_folds=assert_on_folds
            )
        )
    ...
    def PlotEventMatches(source_elist, target_elist, source_in=None, target_in=None):
        return Event.PlotEventMatches(
            source_events=source_elist.events,
            target_events=target_elist.events,
            source_in=source_in,
            target_in=target_in,
        )
    ...
    def getWithFirstEventAt(self, first_event_time):
        return EventList(
            Event.GetWithFirstEventAt(
                events=self.events, first_event_time=first_event_time
            )
        )

    def getWithStartTimesShifted(self, new_start_time):
        return EventList(
            Event.GetWithStartTimesShifted(
                events=self.events, new_start_time=new_start_time
            )
        )
