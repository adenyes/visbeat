...
class Image(AObject):
    ...
    def _splatAtPixCoord(self, im, location=[0, 0], **kwargs):
        ...
        if im.n_channels < 4:
            ...
            return
        if im.n_channels == 4:
            alphamap = np.moveaxis(
                np.tile(im._pixels_float[:, :, 3], (3, 1, 1)), [0], [2]
            )

            blenda = (im._pixels_float[:, :, :3]) * alphamap + self._pixels_float[
                region0[0] : region0[1], region1[0] : region1[1], :
            ] * (1.0 - alphamap)
            if is_int:
    ...
    def writeText(
        ...
        font_paths = find_all_files_with_name_under_path(
            name=font_filename, path=os.path.dirname(os.path.abspath(__file__))
        )
        ...
        if font_size == "fill" and (max_width is not None or max_height is not None):
            font_size = self._get_font_size(text, font_path, max_width, max_height)
    ...
    def _vbmarker(self):
        ...
        if scaleval < 1.0:
            vbms = vbm.getScaled(
                shape=[
                    int(vbm.shape[0] * scaleval),
                    int(vbm.shape[1] * scaleval),
                    vbm.shape[2],
                ]
            )
        else:
        ...
        margin = min(
            vbm.width,
            vbm.height,
            int(VBMMARGIN * self.width),
            int(VBMMARGIN * self.height),
        )

        return dict(
            im=vbms,
            location=[
                self.height - vbms.height - margin,
                self.width - vbms.width - margin,
            ],
        )
    ...
    def writeTextBox(
        ...
        for index, line in enumerate(lines):
            height += text_height
            if place == "left":
                self.writeText((x, height), line, font_filename, font_size, color)
            elif place == "right":
                total_size = self._get_text_size(font_path, font_size, line)
                x_left = x + box_width - total_size[0]
                self.writeText((x_left, height), line, font_filename, font_size, color)
            elif place == "center":
                total_size = self._get_text_size(font_path, font_size, line)
                ...
                total_size = self._get_text_size(
                    font_path, font_size, line_without_spaces
                )
                ...
                for word in words[:-1]:
                    ...
                    word_size = self._get_text_size(font_path, font_size, word)
                    start_x += word_size[0] + space_width
                last_word_size = self._get_text_size(font_path, font_size, words[-1])
