...
class Video(TimeSignal):
    ...
    def initializeBlank(self):
        TimeSignal.initializeBlank(self)
        # YES KEEP
        self.name = None
    ...
    def openVideoWriter(self, output_file_path, fps=None):
        ...
        self.writer = imageio.get_writer(
            output_file_path, "ffmpeg", macro_block_size=None, fps=out_fps
        )
    ...
    def getFrameLinearInterp(self, f):
        if isinstance(f, (int, long)) or f.is_integer():
    ...
    def write(self, output_path, output_sampling_rate=None):
        ...
        tempfilepath = get_temp_file_path(
            final_file_path=output_path, temp_dir_path=self.getTempDir()
        )
        ...
        frame_start_times = np.linspace(
            0, self.getDuration(), num=nsamples, endpoint=False
        )
    ...
    def writeResolutionCopyFFMPEG(self, path, max_height=None):
        ...
        mpc = self.getMPYClip()
        clip_resized = mpc.resize(height=max_height)
    ...
    def writeResolutionCopy(
        self,
        path,
        max_height=None,
        reshape=None,
        input_sampling_rate_factor=None,
        ...
        if input_sampling_rate_factor is not None:
            ...
            self.audio.sampling_rate = (
                self.audio.sampling_rate * input_sampling_rate_factor
            )
        output_path = path
        ...
        for nf in range(len(frame_index_floats)):
            ...
            if reshape == True:
                fim.data = np.reshape(
                    fim.data, (fim.data.shape[1], fim.data.shape[0], fim.data.shape[2])
                )
        ...
        **kwargs
        ...
        print(
            "target start: {}\ntarget end: {}\ntarget duration: {}".format(
                target_start, target_end, target_duration
            )
        )
        ...
        frame_index_floats = np.true_divide(
            np.array(unwarped_target_times), old_frame_time
        )
        tempfilepath = get_temp_file_path(
            final_file_path=output_path, temp_dir_path=Video.VIDEO_TEMP_DIR
        )
        self.openVideoWriter(
            output_file_path=tempfilepath, fps=output_sampling_rate, **kwargs
        )
        start_timer = time.time()
        last_timer = start_timer
        fcounter = 0
        ...
        cropped_output_audio = output_audio.AudioClip(
    ...
    def getWithBeginningCroppedToAudio(self, target):
        ...
        if AB < BA:
            ...
            return self.VideoClip(
                start=BA, end=None, name=self.name + "clipped_to_" + B.name
            )
    ...
    def getWithSoundsOnEvents(
        ...
        **kwargs
        ...
        if output_path is None:
            if self.source is not None:
                output_dir = self.source.getDirForVersion(
                    version_label=self.getInfo("version_label"),
        ...
        if not os.path.isfile(output_path) or force_recompute:
            if isinstance(events, EventList):
                ...
                etimes = events
            new_audio = self.getAudio().getWithSoundAdded(
                sound=event_sound,
                add_times=etimes,
            ...
            audio_clip = MPYAudioArrayClip(reshapex, fps=new_audio.sampling_rate)
            # from a numeric arra
            video_clip = self.getMPYClip()
            video_clip = video_clip.set_audio(audio_clip)
            # video_clip.write_videofile(output_path, codec='libx264', write_logfile=False)
            MPYWriteVideoFile(
                video_clip, output_path, codec="libx264", write_logfile=False
            )
    ...
    def CreateFromVideoAndAudio(
        ...
        **kwargs
        ...
        assert not (
            video_path and video_object
        ), "provided both video path and object to CreateFromVideoAndAudio"
        assert not (
            audio_path and audio_object
        ), "provided both audio path and object to CreateFromVideoAndAudio"
        ...
        if clip_to_video_length:
            ...
            if n_audio_samples_in_vid < n_audio_samples_sig:
                ...
                if n_audio_samples_in_vid > n_audio_samples_sig:
                    nreps = math.ceil(
                        truediv(n_audio_samples_in_vid, n_audio_samples_sig)
                    )
                    if is_stereo:
                        audio_sig = np.concatenate(
                            (
                                audio_sig,
                                np.zeros(
                                    (2, n_audio_samples_in_vid - n_audio_samples_sig)
                                ),
                            ),
                            axis=1,
                        )
        ...
        if is_stereo:
            ...
            audio_clip = MPYAudioArrayClip(reshapex, fps=audio_sampling_rate)
            # from a numeric arra
        else:
            reshapex = audio_sig.reshape(len(audio_sig), 1)
            reshapex = np.concatenate((reshapex, reshapex), axis=1)
    ...
    def CreateByStackingVideos(
        ...
        **kwargs
        ...
        tempfilepath = get_temp_file_path(
            final_file_path=output_path, temp_dir_path=vids[0].getTempDir()
        )
    ...
    @staticmethod
    def CreateFromVideoAndAudioPaths(
        video_path, audio_path, output_path, return_vid=True, **kwargs
    ):
        return Video.CreateFromVideoAndAudio(
            video_path=video_path,
            audio_path=audio_path,
            output_path=output_path,
            return_vid=return_vid,
            **kwargs
        )

    @staticmethod
    def CreateFromVideoAndAudioObjects(
        video, audio, output_path, clip_to_video_length=True, return_vid=True, **kwargs
    ):
        return Video.CreateFromVideoAndAudio(
            video_object=video,
            audio_object=audio,
            output_path=output_path,
            clip_to_video_length=clip_to_video_length,
            return_vid=return_vid,
            **kwargs
        )
    ...
    def getFeatureFunctionsList(self):
        return (
            super(Video, self).getFeatureFunctionsList()
            + self.audio.getFeatureFunctionsList()
        )
    ...
    def getFeature(self, name, force_recompute=False, **kwargs):
        ...
        assert not kwargs.get(
            "params"
        ), "STILL TRYING TO USE PARAMS INSTEAD OF KWARGS. FIX THIS"
        ftry = super(Video, self).getFeature(
            name=name, force_recompute=force_recompute, **kwargs
        )
        if ftry is not None:
            return ftry
        else:
            return self.audio.getFeature(
                name=name, force_recompute=force_recompute, **kwargs
            )
...
if BeatGUI.VIEWER_INSTALLED:
    ...
    def runBeatGUIOnAudio(self):
        audio = self.getAudio()
        self.gui.run(
            local_saliency=audio.getLocalRhythmicSaliency(),
            frame_rate=audio._getFrameRate(),
